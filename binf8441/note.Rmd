...
##### variable selection
  ...
  1. Forward selection: start from the simplest model with no corvariate x, and add by hypotheses test (between simpler and more complex model) unitil we cannot reject $H_0$ $Y=\beta_0$ $H_1$ $Y=\beta_0+\beta_1 X_1$ lrt {y with different means but same variance} until cannot reject null hypotheses
    ...
    $L_0=\prod{f(y_i|\beta_0 \sigma^2)}=\prod{\frac{1}{\sqrt(2\pi\sigma^2)} \exp{-\frac{(y_i-\widehat{\beta}_0)^2}{2\sigma^2}}}$ $y_i$ ~ $Normal(\beta_0, \sigma^2)$  $\widehat{beta_0}_{mle}=$ the sample average of $Y_i$  $\widehat{\sigma^2}_{mle}=$ the sample variance of $Y_i$

    $L_1=\prod{f(y_i|\beta_0 \beta_1 x_i \sigma^2)}=\prod{\frac{1}{\sqrt(2\pi\sigma^2)} \exp{-\frac{(y_i-\widehat{\beta}_0-\widehat{\beta_1} x_{1i})^2}{2\sigma^2}}}$ $y_i$ ~ $Normal(\beta_0+\beta_1 x_1, \sigma^2)$ $x_1$ are different for each sample
    ...
    statistics function of only data not parameter  $\prod{f(y_i|\beta_0 \beta_1 x_i \sigma^2)}$ not statistics $\prod{\frac{1}{\sqrt(2\pi\sigma^2)} \exp{-\frac{(y_i-\widehat{\beta}_0-\widehat{\beta_1} x_{1i})^2}{2\sigma^2}}}$  statistics
